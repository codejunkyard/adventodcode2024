...
//         let indegree_table = build_indegree_table(&rules);
...
//                 if let Some(rule_vec) = rule {
//                     if clone.iter().any(|&x| rule_vec.contains(&x)) {
//                         sum += fix_order(update, &rules, &indegree_table);
//                         break;
//                     }
//                 }
//             }
...
//             indegree_table: &HashMap<i32, usize>,
//         ) -> i32 {
//             println!("{:?}", update);
...
// fn build_indegree_table(rules: &HashMap<i32, Vec<i32>>) -> Vec<i32> {
//     let mut in_degree: HashMap<i32, usize> = HashMap::new();

//     // Step 1: Iterate through the rules to calculate in-degrees
//     for (_, dependencies) in rules {
//         // Destructure the key-value pair
//         for &dep in dependencies {
//             // Increment the in-degree of each dependency
//             *in_degree.entry(dep).or_insert(0) += 1;
//         }
//     }

//     // Step 2: Ensure that all nodes from the rules are included in the in_degree map, even if they have in-degree 0
//     for &node in rules.keys() {
//         in_degree.entry(node).or_insert(0); // This inserts 0 if the node is not present
//     }

//     if DEBUG {
//         for (node, degree) in &in_degree {
//             println!("Node: {}, In-Degree: {}", node, degree);
//         }
//     }
